...
public class PixelImage
{
  ...
  public Pixel[][] weightedAverage( final int[][] weight, final int scale)
  {
    Pixel[][] referenceData = this.getData();
    ...
    for (int row = 1; row < this.getHeight() - 1; row++) 
    {
      for (int col = 1; col < this.getWidth() - 1; col++) 
      {
        for(int i = 0; i<3;i++)
        {
          for(int j = 0; j<3;j++)
          {
            weightedSumRed += (referenceData[row + (i - 1)][col + (j - 1)].red * weight[i][j]);
            weightedSumBlue += (referenceData[row + (i - 1)][col + (j - 1)].blue * weight[i][j]);
            weightedSumGreen += (referenceData[row + (i - 1)][col + (j - 1)].green * weight[i][j]);
          }
        }
        data[row][col].red = this.limiter(weightedSumRed / scale);
        data[row][col].blue = this.limiter(weightedSumBlue / scale);
        data[row][col].green = this.limiter(weightedSumGreen / scale);
        weightedSumRed = 0;
        weightedSumBlue = 0;
        weightedSumGreen = 0;
      }
  ...
  public int limiter(int colorValue)
  {
    if(colorValue < 0)
    {
      colorValue = 0;
